...
def baidu_tieba(url,current_page='/page/1',x=1):
    ...
    if url == bdurl:
        ...
        reg_img = r"""img\salt=?"?\s?.+src\s*=\s*"(\S+)\.(\w+)".+data-highres"""
    '''
    else:
    ...
    if nextpage!=[]:
        if nextpage[0] == nextpage[-1] and nextpage[0] != "/page/2":
            ...
            print nextpage[0].split('/')[-1] + "  " + nextpage[-1].split('/')[-1]
            if int(nextpage[0].split('/')[-1]) > int(nextpage[-1].split('/')[-1]):
            	x=baidu_tieba(url,nextpage[0])
            else:
